...
class FileStore {
    ...
    public storeSpawnUnmarshalledBegin = async (isSpawn: boolean, address: number, length: number): Promise<void> => {
        const arrayBuffer = windowAny.Module.HEAPU8.subarray(address, address + length);
        const array = new Uint8Array(arrayBuffer);
        const filename = isSpawn ? "spawn.mpq" : "diabdat.mpq";
        await this.store.set(filename, array);
